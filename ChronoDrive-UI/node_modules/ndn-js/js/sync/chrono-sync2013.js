...
ChronoSync2013.prototype.publishNextSequenceNo = function(applicationInfo)
{
  ...
  console.log('sending interest: ', interest.getName().toUri());
...
{
  console.log('sync received interest: ', interest.getName().toUri());
  console.log('size: ', interest.getName().size());
  console.log('sync applicationBroadcastPrefix', this.applicationBroadcastPrefix.toUri());
  if (!this.enabled)
  ...
  if (interest.getName().size() == this.applicationBroadcastPrefix.size() + 2) {
    ...
    console.log('syncdigest in onInterest: ', syncdigest)
    console.log('current digest root: ', this.digest_tree.getRoot());
  }
  if (interest.getName().size() == this.applicationBroadcastPrefix.size() + 2 || syncdigest == "00") {
    console.log('sync thinks it\'s a recovery instance/first broadcast...');
    console.log('sync called processRecoveryInst...');
    ...
    if (syncdigest != this.digest_tree.getRoot()) {
      console.log('digests didn\'t match...');
      var index = this.logfind(syncdigest);
      var content = [];
      if(index == -1) {
        console.log('couldn\'t find syncdigest in digest log...');
        var self = this;
        ...
        console.log('sync called processSyncInst...');
...
{
  console.log('sync onData recieved interest: ', interest.getName().toUri());
  if (!this.enabled)
    // Ignore callbacks after the application calls shutdown().
    return;
  ...
  try {
    ...
    console.log('onData - finished calling onReceivedSyncState...')
  } catch (ex) {
  ...
  console.log('onData - sent new state broadcast')
  this.face.expressInterest(interest, this.onData.bind(this), this.syncTimeout.bind(this));
};
...
ChronoSync2013.prototype.processRecoveryInst = function(interest, syncdigest, face)
{
  if (this.logfind(syncdigest) != -1) {
    ...
    console.log('processRecoveryInst content: ', content);
    ...
    if (content.length != 0) {
      ...
      this.keyChain.sign(co, this.certificateName, function() {
        try {
          console.log('processRecoveryInst putting data on face for interest ' + interest.getName().toUri() + ': ', content_t);
          face.putData(co);
        } catch (e) {
...
ChronoSync2013.prototype.processSyncInst = function(index, syncdigest_t, face)
{
  ...
  console.log('sync.processSyncInst() making SyncStates...');
  ...
  if (content.length != 0) {
    ...
    this.keyChain.sign(co, this.certificateName, function() {
      try {
        console.log('processSyncInst putting SyncStateMsg data on face...', content_t);
        face.putData(co);
      }
      catch (e) {
        console.log(e.toString());
...
ChronoSync2013.prototype.judgeRecovery = function(interest, syncdigest_t, face)
{
  ...
  if (index != -1) {
    if (syncdigest_t != this.digest_tree.root)
      console.log('calling processSyncInst() from judgeRecovery');
