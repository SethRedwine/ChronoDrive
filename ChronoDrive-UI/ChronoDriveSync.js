...
var ChronoDriveSync = function (userName, lastUpdated, userDirChecksum, hubPrefix, face, keyChain, certificateName, roster) {
...
ChronoDriveSync.prototype.onInterest = function (prefix, interest, face, interestFilterId, filter) {
    ...
    // TODO: Actually, here we should iterate through the file cache/grab the right file, then check the checksum and the timestamp and update if it's older
    // QUESTION: How do we ensure that files that aren't created on one system, and therefore wouldn't have interests are created?
    // Maybe we'll need a hierarchical structure instead of a map - again, maybe it WILL need to be the root FileElement
...
function initiateChat() {
    ...
    ndn_js_1.Interest.setDefaultCanBePrefix(true);
    ...
    var face = new ndn_js_1.Face(new ndn_js_1.UnixTransport());
    // Set up the KeyChain.
    var keyChain = new ndn_js_1.KeyChain("pib-memory:", "tpm-memory:");
    // This puts the public key in the pibImpl used by the SelfVerifyPolicyManager.
    keyChain.importSafeBag(new ndn_js_1.SafeBag(new ndn_js_1.Name("/testname/KEY/123"), new ndn_js_1.Blob(DEFAULT_RSA_PRIVATE_KEY_DER, false), new ndn_js_1.Blob(DEFAULT_RSA_PUBLIC_KEY_DER, false)));
    face.setCommandSigningInfo(keyChain, keyChain.getDefaultCertificateName());
    // TODO: Set some okay defaults here
    var chronoDriveSync = new ChronoDriveSync(screenName, 0, '', HUB_PREFIX, face, keyChain, keyChain.getDefaultCertificateName(), []);
